...
var eduDefineBaseDir = hexo.config.edudefine_dir;


var deployDir = pathFn.join(baseDir, '.deploy_appengine');

var eduDefineDir = pathFn.join(eduDefineBaseDir, 'blog');
var deployPubDir = pathFn.join(deployDir, 'blog');
...
hexo.extend.deployer.register('edudefine', function(args) {
...
// 
  ...
  return fs.exists(eduDefineDir).then(function(exists) {
    if (!exists) return;
    log.info('Clearing old EduDefine Blog folder...');
    log.info(eduDefineDir);
    return fs.emptyDir(eduDefineDir);
  }).then(function() {
    log.info('Copying files from public folder...');
     log.info(publicDir);
    return fs.copyDir(publicDir, eduDefineDir);
  }).then(function() {
        log.info('Copied to Blog in EduDefineWebsite');
      });
